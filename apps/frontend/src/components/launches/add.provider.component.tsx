    ...
    <div className="flex flex-col gap-[16px] pt-[8px]">
      <p className="text-[14px] text-textColor/80">
        {t(
          ...
          'The Omnia Social browser extension is not installed. You need to install it before connecting this channel.'
        )}
      </p>
  ...
  return (
    <div className="flex flex-col gap-[16px] pt-[8px]">
      ...
      <ul className="flex flex-col gap-[8px] list-disc ps-[20px] text-[14px] text-textColor/80">
        ...
        <li>
          Omnia Social does not take responsibility for any issues arising or
...
export const AddProviderComponent: FC<{
  social: Array<{
    ...
    configured: boolean;
    approvalStatus?: 'waiting-for-approval' | 'not-configured';
                ...
                <Web3Providers
                  onComplete={(code, newState) => {
                    window.location.href =
                      ...
                      `/integrations/social/${identifier}?code=${code}&state=${newState}${
                        onboarding ? '&onboarding=true' : ''
                      }`;
                  }}
                  nonce={url}
        ...
        const gotoIntegration = async (externalUrl?: string) => {
          ...
          if (window.self !== window.top) {
            const popup = window.open(
              url,
              '_blank',
              'popup=yes,width=680,height=800'
            );
            if (popup) {
              try {
          ...
          try {
            ...
            toaster.show(
              t(
                ...
                'Omnia Social browser extension is not installed or not reachable.'
              ),
              'warning'
            );
            return;
              ...
              return (
                <div
                  ...
                  data-provider-availability={
                    unavailable ? item.approvalStatus : 'configured'
                  }
                  {...(!!item.toolTip
                    ? {
                        'data-tooltip-id': 'tooltip',
                ...
                >
                  ...
                  >
                    {item.name}
                    {unavailable && (
                      <span className="block text-[11px] leading-[14px] text-yellow-400">
