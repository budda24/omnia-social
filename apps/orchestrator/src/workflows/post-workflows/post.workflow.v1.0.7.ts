...
export async function postWorkflowV107({
  ...
  let postsListBefore = await getPostsList(organizationId, postId);
  let [post] = postsListBefore;
  ...
  for (;;) {
    // The wait can last hours. Re-read the channel before every claim so a
    // reconnect requirement or disable that happened during the wait wins.
    ...
    const slot = await claimPublishSlot(
      organizationId,
      post.integration.id,
      post.id,
      new Date()
    ...
    if ('stopReason' in slot) {
      await changeState(post.id, 'ERROR', slot.stopReason, postsListBefore);
      return;
    }

    const deferUntil = temporalDate(slot.deferUntil);
    const deferred = await deferPost(post.id, deferUntil, slot.reason, postNow);
    if (deferred.status === 'ceiling') {
      const maxUntil = temporalDate(deferred.maxUntil);
      const message = `${
        slot.reason
      }. The next safe slot ${deferUntil.toISOString()} is after the ${maxUntil.toISOString()} deferral limit.`;
      await changeState(post.id, 'ERROR', message, postsListBefore);
      await inAppNotification(
        post.organizationId,
        `We couldn't safely delay your ${post.integration.providerIdentifier} post`,
        message,
        true,
        false,
        'fail'
    ...
    const deferredPublishDate = temporalDate(deferred.publishDate);

    if (deferred.firstDeferral) {
      await inAppNotification(
        post.organizationId,
        `Your ${post.integration.providerIdentifier} post was moved to the next safe slot`,
        `${
          slot.reason
        }. It is now scheduled for ${deferredPublishDate.toISOString()}.`,
        true,
        false,
        'info'
      );
    ...
    await sleep(
      Math.max(0, deferredPublishDate.getTime() - new Date().getTime())
    );
    postsListBefore = await getPostsList(organizationId, postId);
    [post] = postsListBefore;
    if (!post) {
      await changeState(postId, 'ERROR', 'No Post');
      return;
    }
